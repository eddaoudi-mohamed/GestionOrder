...
import HistoryOrderCardBody from "./HIstoryOrderCardBody";

interface HistoryOrderCardProps {
  historyOrd: History;
}

const HistoryOrderCard = ({historyOrd} : HistoryOrderCardProps) => {

  const severityClasses = getSeverityColors(historyOrd.action_type);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 px-6 py-3 text-meta-4 ">
        {historyOrd.action_type === "refunded" ? (
          <span
            className={`material-symbols-outlined ${getHistorySeverity(historyOrd.action_type)}`}
          >
            currency_exchange
          </span>
        ) : (
          <>{getActionTypeIcon(historyOrd.action_type)}</>
        )}
        <p className="text-xl font-semibold uppercase">
          {historyOrd.initiator}
        </p>
      </div>
      <div className="p-6">
        ...
        <HistoryOrderCardBody history={historyOrd} />
        <div
